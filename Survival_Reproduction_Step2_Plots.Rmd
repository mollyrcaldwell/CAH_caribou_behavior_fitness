...
```{r}
# early behavior and fitness data
early_data <- read.csv("./Code_Output/early_behavior_fitness.csv")

# late behavior and fitness data
late_data <- read.csv("./Code_Output/late_behavior_fitness.csv")

# early behavior models - survival (output from survival model step 1 code)
early_surv_mod <- readRDS("./Code_Output/early_cond_surv_sig_models.rds")

# late behavior models - survival (output from survival model step 1 code)
late_surv_mod <- readRDS("./Code_Output/late_cond_surv_sig_models.rds")

# early behavior models - reproduction (output from reproduction model step 1 code)
early_repro_mod <- readRDS("./Code_Output/early_cond_part_sig_models.rds")

# late behavior models - reproduction (output from reproduction model step 1 code)
...
late_surv_HR_bm <- late_surv_pred %>% 
  filter(beh_var == "Seasonal range area (km^2)" & cond_int == "avg_biom_herd" & predictor_names == "avg_biom_herd" & condition_level %in% c("max_quantity", "min_quantity"))

late_surv_HR <- bind_rows(late_surv_HR, late_surv_HR_de, late_surv_HR_bm)
...
late_repro_HR_bm <- late_repro_pred %>% 
  filter(beh_var == "Seasonal range area (km^2)" & cond_int == "avg_biom_herd" & predictor_names == "avg_biom_herd" & condition_level %in% c("max_quantity", "min_quantity"))

late_repro_HR <- bind_rows(late_repro_HR, late_repro_HR_de, late_repro_HR_bm)
...
```{r}
# early summer: survival - avg biom ^2, repro - avg biom * mosq, repro - resource breadth * dn
...
# late summer: survival - forage breadth * biom, repro - avg biom * fly, avg biom * biom, forage breadth * de, forage breadth * biom
...
early_surv_fb_plot <- ggplot(data.frame(x = 0, y = 0), aes(x, y)) +
  ...
  labs(x = "Forage use breadth", y = "Annual survival probability") +
  scale_y_continuous(breaks = c(0, 0.5, 1)) +
  theme_classic() +
...
late_surv_biom_plot <- ggplot(data.frame(x = 0, y = 0), aes(x, y)) +
  ...
  labs(x = expression("Forage biomass use (g/m"^2*")"), y = "Annual survival probability", title = "B. Late summer") +
  scale_y_continuous(breaks = c(0, 0.5, 1)) +
  theme_classic() +
...
# combine and save plots
left_col <- (early_biom2_plot / early_surv_fb_plot) +
  plot_layout(axes = "collect_y")

left_mid_col <- (early_biom_repro_plot / early_rb_repro_plot) +
  plot_layout(axes = "collect_y")


right_mid_col <-  late_surv_biom_plot / late_fb_surv_plot + plot_layout(axes = "collect_y")
  

right_col <- (late_biom_biom_repro_plot / late_biom_fly_repro_plot /
                late_fb_repro_plot) +
  plot_layout(heights = c(0.4, 0.4, 1.1), axes = "collect_y")
