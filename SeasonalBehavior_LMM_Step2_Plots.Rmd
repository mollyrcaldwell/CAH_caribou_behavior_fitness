...
# early behavior models (output from behavior model step 1 code)
...
# late behavior models (output from behavior model step 1 code)
...
# add dummy row for forage quality color & longitude to show
dummy <- data.frame(predictor_type = "Forage quality", response = "Seasonal range longitude", label = "No effect", predictor_value = 0, fit = -148)
...
space_data_early <- space_data_early %>% 
  group_by(response) %>% 
...
plot_early_space <- ggplot(space_data_early) +
    ...
    geom_ribbon(aes(x = predictor_value,
                    ...
                    fill = predictor_type), 
                alpha = 0.2) +
  geom_text(aes(x = 0, y = mean_fit, label = label), size = 5) +
...
# combine plots
plot_space <- (((plot_early_space + labs(title = "A. Early summer") + 
                                   theme(title = element_text(size = 8)))) | (plot_late_space + labs(title = "B. Late summer") +
